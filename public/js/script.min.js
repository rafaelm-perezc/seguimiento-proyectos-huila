document.addEventListener('DOMContentLoaded', () => {
    ...
    const txtAvanceGeneral = document.getElementById('txtAvanceGeneral');
    const txtObservacionesGeneral = document.getElementById('txtObservacionesGeneral');
    const modoAvanceGeneral = document.getElementById('modoAvanceGeneral');
    const modoAvanceMixto = document.getElementById('modoAvanceMixto');
    const modoAvanceSede = document.getElementById('modoAvanceSede');
    const generalAdvanceFields = document.getElementById('generalAdvanceFields');
    ...
    function toggleModoAvance() {
        const isGeneral = modoAvanceGeneral.checked;
        generalAdvanceFields.classList.toggle('hidden', !isGeneral);
        txtAvanceLocal.disabled = isGeneral;
        txtObservacionesLocal.disabled = isGeneral;
    }

    modoAvanceGeneral.addEventListener('change', toggleModoAvance);
    modoAvanceSede.addEventListener('change', toggleModoAvance);
    toggleModoAvance();
    ...
    function toggleModoAvance() {
        const isGeneral = modoAvanceGeneral && modoAvanceGeneral.checked;
        const isMixto = modoAvanceMixto && modoAvanceMixto.checked;
        if (generalAdvanceFields) {
            generalAdvanceFields.classList.toggle('hidden', !(isGeneral || isMixto));
        }
        if (txtAvanceLocal) txtAvanceLocal.disabled = isGeneral;
        if (txtObservacionesLocal) txtObservacionesLocal.disabled = isGeneral;
    }

    if (modoAvanceGeneral && modoAvanceSede) {
        modoAvanceGeneral.addEventListener('change', toggleModoAvance);
        if (modoAvanceMixto) modoAvanceMixto.addEventListener('change', toggleModoAvance);
        modoAvanceSede.addEventListener('change', toggleModoAvance);
        toggleModoAvance();
    }
    ...
    btnAddLocation.addEventListener('click', () => {
        ...
        const isMixto = modoAvanceMixto && modoAvanceMixto.checked;
        if (isGeneral) {
            if (!txtAvanceGeneral || txtAvanceGeneral.value === '') {
                return Swal.fire('Falta Avance', 'Ingresa el % de avance general', 'warning');
            }
        } else if (txtAvanceLocal.value === '') {
            if (!isMixto || !txtAvanceGeneral || txtAvanceGeneral.value === '') {
                return Swal.fire('Falta Avance', 'Ingresa el % de avance', 'warning');
            }
        }
        ...
        const avanceValue = isGeneral
            ? txtAvanceGeneral.value
            : (txtAvanceLocal.value || (isMixto && txtAvanceGeneral ? txtAvanceGeneral.value : ''));
        const observacionesValue = isGeneral
            ? (txtObservacionesGeneral ? txtObservacionesGeneral.value : '')
            : (txtObservacionesLocal.value || (isMixto && txtObservacionesGeneral ? txtObservacionesGeneral.value : ''));
        ...
        const locationObj = {
            ...
            avance: avanceValue,
            observaciones: observacionesValue ? observacionesValue.toUpperCase() : ''
        }
    ...
    function resetForm() {
        ...
        if (modoAvanceSede) modoAvanceSede.checked = true;
        if (txtAvanceGeneral) txtAvanceGeneral.value = '';
        if (txtObservacionesGeneral) txtObservacionesGeneral.value = '';
        toggleModoAvance();
    }
                ...
                .then(d => {
                    ...
                    if (d.length) {
                        d.forEach(p => {
                            ...
                            li.onclick = () => selectProject(p.codigo_bpin);
    ...
    function selectProject(bpin) {
        ...
        fetch(`/api/project/${bpin}`)
            ...
            .then(d => {
                ...
                registeredLocations = d.locations || [];
                renderLocationsTable();
            });
    }
